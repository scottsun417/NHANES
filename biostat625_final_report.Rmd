...
After all the data cleaning steps, we found that our final dataframe has 9236 rows and 126 columns left. Our response variable DIQ010 has distribution looks like figure1, where 0 means doctor had never told a patient that he had diabetes and 1 menas the opposite. We observed that the number of negative responses are way more than positive results, which indicates that this dataset is imbalanced. Imbalanced data set means that the classes in the dataset have highly uneven sample sizes, and is often seen in healthcare datasets. We ran a prediction using XGBClassifier and the classification result from the confusion matrix in figure2 also shows that the true prediction in class 1 is 1, which is much less than the true prediction in class 0, which is 1622. In order to solve this problem, we introduce the method of SMOTE (Synthetic Minority Oversampling Technique). The basic idea of SMOTE is that we take samples of the feature space for each target class and its nearest neighbors, then generates new examples that combine features of the target case with features of its neighbors to increase the number of cases in the minority class (likebupt & PeterCLu 2021). After using SMOTE, we ran the XGBClssifier prediction method again, and we observed  that the result in the confusion matrix is more reasonable this time (figure3).

Since we have used the SMOTE technique to make our dataset balanced, we can now proceed to feature selection. Given that our dataset has 126 columns as predictors, it is very important to select the most important features and fit them in our models. Here we used XGBoost to select features because XGBoost can automatically calculate the importance of features. After we ran XGBoost, top 24 features sorted by importance are listed here (figure4). The correlations of these features are shown as a heatmap in figure4.

After we split our dataframe (after oversampling with SMOTE) into train and test groups, we first used logistic regression method to train our model and get predictions. Since logistic regression is the most basic statistical methods in machine learning to get predictions of binary outcomes, one of our goals is to compare different other machine learning methods with logistic regression to see whether those methods have higher efficiency and better performance. The results are shown in figure 5. From this table, we noticed that the accuracy score is about 0.773, which means only 77.3% of predictions made by the model are correct. In the confusion matrix, we observed that the number of cases when both prediction result and true data are negative is 1620, and the number of cases when both prediction result and true data are positive is 165. This result is of great significance because if we have future data contains all predictors, we can use them to predict the distribution of our response variable DIQ010. In reality, this means we can predict whether a patient will have diabetes based on the 24 selected features. \
We then used other machine learning techniques such as random forest, gradient boosting, adaptive boosting and etc. to get accuracy scores and confusion matrices. We compared these models by comparing their accuracy scores. The result is shown in figure 6. We found that bagging decision tree has the highest accuracy score followed by random forest and bagging kneighbors.\
Another useful way to compare models is through the AUC-ROC curve. AUC-ROC curve is a performance measurement for the classification problems at various threshold settings. ROC is a probability curve and AUC represents the degree or measure of separability. It tells how much the model is capable of distinguishing between classes (Sarang 2018). In this study, we selected 5 models which have accuracy scores higher than logistic regression, and plot their AUC-ROC curves (figure7). We found that random forest has the highest AUC value 0.97, which means random forest is the best at distinguishing whether the patient had diabetes or not. 
